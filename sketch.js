...
function startGame() {
    ...
    scrubber = 100;
    options = ['mountain', 'tree', 'collectable', 'cloud', 'canyon', 'enemy', 'cloud', 'cloud', 'trees', 'mountain'];
    ...
    objects = [clouds, mountains, canyons, trees, collectables, [flagpole], enemies];
...
function draw() {
    ...
    if (cloudOver) {
        ...
        clouds.push(new Cloud(gameChar.scrollPos - 40, 100 + ceil(random(50, 100))));
        clouds.push(new Cloud(gameChar.scrollPos - 180, 100 + ceil(random(50, 100))));
    }
    pop();
...
class Enemy {
    ...
    drawEnemy() {
        ...
        fill(100, 200, 255);
        quad(x_1 - 5, y_1 + 5, x_1 + 5, y_1 + 5, this.x + x_t + 5, this.y + y_t + 5, this.x + x_t - 5, this.y + y_t + 5);
        quad(x_2 - 5, y_2 + 5, x_2 + 5, y_2 + 5, this.x + x_t + 5, this.y + y_t + 5, this.x + x_t - 5, this.y + y_t + 5);
        fill(255, 0, 0)
        ...
        for (var k = 0; k < 10; k++) {
            triangle(x_1 + (k * w / 10), y_1, x_1 + ((k + 1) * w / 10), y_1, x_1 + ((2 * k + 1) / 2 * w / 10), y_1 + 10);
        }

        fill(90, 100, 90);
        strokeWeight(2);
        stroke(51);
        curve(this.x - curve_x / 2, this.y + curve_y, this.x - curve_x / 4, this.y + curve_y / 4, this.x + curve_x / 4, this.y + curve_y / 4, this.x + curve_x / 2, this.y + curve_y / 4);
        curve(this.x - curve_x / 2, this.y + curve_y + curve_y / 4, this.x - curve_x / 4, this.y + curve_y / 4, this.x + curve_x / 4, this.y + curve_y / 4, this.x + curve_x / 2, this.y + curve_y / 4);
        fill(0, 0, 255)
        ellipse(e1x, e1y, 4, 4);
        ellipse(e2x, e2y, 4, 4);
        pop();
    ...
    check() {
        ...
        if (this.base > gameChar.y - 65 && gameChar.x_world > this.x - this.w / 2 && gameChar.x_world < this.x + this.w / 2 && !game.halt && !game.gameOver) {
...
class Collectable {
    ...
    check() {
        if (abs(gameChar.x_world - this.x) < this.size / 2 && abs(gameChar.y - this.y) <= this.size / 2 && this.isFound == false) {
            this.isFound = true;
...
class Canyon {
    ...
    check() {
        var condition = (gameChar.x_world > this.x) && (gameChar.x_world < (this.x + this.width)) && (gameChar.y == this.y);
